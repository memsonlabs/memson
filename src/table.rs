...
impl Db {
    ...
    fn eval_write(&mut self, cmd: WrCmd) -> Result<(), Error> {
        match cmd {
            WrCmd::Insert(key, rows) => {
                self.insert(key, rows)
            }
        }
    }

    pub fn insert(&mut self, key: String, rows: Vec<JsonObj>) -> Result<(), Error> {
    ...
    fn get_mut(&mut self, key: &str) -> Option<&mut Table> {
...
impl Collections {
    ...
    fn from(colls: BTreeMap<String, Table>) -> Self {
        Self { colls }
    }

    fn get(&self, key: &str) -> Option<&Table> {
        self.colls.get(key)
    }

    fn set<K: Into<String>>(&mut self, key: K, coll: Table) {
        self.colls.insert(key.into(), coll);
    }

    pub(crate) fn insert<K: Into<String>>(&mut self, key: K, rows: Vec<JsonObj>) {
        let coll = self.colls.entry(key.into()).or_insert_with(|| Table::new());
        coll.extend(rows);
    }
...
impl Table {
    ...
    pub fn from(rows: Vec<JsonObj>) -> Self {
        let id = rows.len();
        Self { rows, id }
    }
